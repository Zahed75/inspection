...
class ResultHeader extends StatefulWidget {
  const ResultHeader({
    ...
    required this.siteName,
    required this.timestamp,
    required this.totalScore, // might be obtained in caller
    required this.maxScore,   // might be total in caller
...
class _ResultHeaderState extends State<ResultHeader>
  ...
  Widget build(BuildContext context) {
    ...
    // Always display obtained/total (e.g., 30/50), even if inputs arrive swapped
    final obtainedForDisplay = min(widget.totalScore, widget.maxScore);
    final totalForDisplay = max(widget.totalScore, widget.maxScore);
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(widget.siteName ?? 'Unknown Site',
                        style: theme.textTheme.bodyLarge?.copyWith(
                          color: Colors.grey[600],
                          fontWeight: FontWeight.w500,
                        ),
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Text(date, style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey[600])),
                        ],
                      ),
                      const SizedBox(height: 12),
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('$obtainedForDisplay/$totalForDisplay',
                              style: theme.textTheme.bodyLarge?.copyWith(
                                color: Colors.grey[800],
                                fontWeight: FontWeight.w700,
                              ),
      ...
      child: Stack(
        ...
        children: [
          ...
          SizedBox(
            width: 84, height: 84,
            child: CircularProgressIndicator(
              value: _animation.value.clamp(0.0, 1.0), strokeWidth: 6,
              backgroundColor: Colors.transparent,
              valueColor: AlwaysStoppedAnimation<Color>(scoreColor),
            ),
          ),
  ...
  Color _getScoreColor(double percent) {
    if (percent >= 0.8) return const Color(0xFF10B981); // Emerald
    if (percent >= 0.6) return const Color(0xFFF59E0B); // Amber
    if (percent >= 0.4) return const Color(0xFFEF4444); // Red
    return const Color(0xFF6B7280); // Gray
  }
