...
//
//     // Keep UI logic as-is: a single category for the screen
...
//   }
//
//   /// NEW: get survey title best-effort from result (doesn't affect UI)
...
//   ) async {
//     // screen data (kept), only for header totals/feedback
//     final data = _processSurveyData(result);
//
//     // build proper category groups just for PDF
...
//     final surveyTitle = _surveyTitleFrom(result);
...
//
//           // Build a section per real category
...
//                     final rowCat = _qCategory(q).isNotEmpty
//                         ? _qCategory(q)
//                         : catName;
//
//                     return pw.TableRow(
...
//                       children: [
//                         _cell('${i + 1}'),
//                         _cell(rowCat), // Category column — fixed
//                         _cell(qText),
//                         _cell(qAns),
//                         _cell(
...
//                           alignEnd: true,
//                         ),
//                         _cell(catRemark), // Remarks column — per category
//                       ],
//                     );
//                   }),
//                 ],
...
//           ),
//           pw.SizedBox(height: 6),
//           pw.Text(overallFeedback, style: const pw.TextStyle(fontSize: 11)),
//         ],
...
class _ResultScreenState extends ConsumerState<ResultScreen> {
  ...
  Map<String, dynamic> _processSurveyData(SurveyResultModel result) {
    ...
    final resolvedSiteCode =
    (result.outletCode?.trim().isNotEmpty == true)
    ...
    final resolvedSurveyTitle =
    (result.surveyTitle?.trim().isNotEmpty == true)
        ...
        data: (result) {
          ...
          final rawObt =
              (processedData['overall']?['obtainedMarks'] as num?)
                  ...
                  0.0;
          final rawTot =
              (processedData['overall']?['totalMarks'] as num?)
                  ?.toDouble() ??
                  0.0;
          final obtainedForCalc = rawObt <= rawTot ? rawObt : rawTot;
          final totalForCalc = rawTot >= rawObt ? rawTot : rawObt;
          ...
          return DefaultTabController(
            ...
            child: Column(
              children: [
                ...
                ResultHeader(
                  siteCode: siteCode,
                  siteName: siteName, // now survey title; null hides line
                  timestamp: timestamp,
                  totalScore: obtainedForCalc.round(),
                  maxScore: totalForCalc.round(),
                  percent: percent,
